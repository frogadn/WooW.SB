...
namespace WooW.SB.BlazorGenerator.BlazorComponents.BlazorInputs.WoSpinEdit
{
    public class WoSpinEditView
    {
        ...
        /// </summary>
        private StringBuilder _spintEdit = new StringBuilder();
        ...
        public WoSpinEditView(string _identMethodsAndProperties)
        {
        ...
        public string GetCode(WoItem item)
        {
            ...
            _spintEdit.AppendLine($@"{_identMethodsAndProperties}#region {item.BindedProperty}");

            _spintEdit.AppendLine(BuildTag());

            _spintEdit.AppendLine(BuildComponent());

            _spintEdit.AppendLine(BuildContainerComponent());

            _spintEdit.AppendLine(BuildAlertComponent());

            _spintEdit.AppendLine($@"{_identMethodsAndProperties}#endregion {item.BindedProperty}");

            _partialReady.Details = $@"Se creo la view para el componente: {item.BindedProperty}.";
            _observer.SetLog(_partialReady);

            return _spintEdit.ToString();
        }
        ...
        {
            return $@"
{_identMethodsAndProperties}// Este código fue generado por el fichero WoSpinEditView.cs en el Path: WooW.SB\BlazorGenerator\BlazorComponents\BlazorInputs\WoSpinEdit\WoSpinEditView.cs.
...
{_identMethodsAndProperties}public WoInputAlertView {_item.BindedProperty}Alert {{ get; set; }} = new WoInputAlertView();";
        ...
        private WoLog _partialReady = new WoLog()
        {
            ...
            {
                Class = "WoSpinEditView",
                MethodOrContext = "GetSpinEditView"
            }
        };
