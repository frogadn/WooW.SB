...
{
    public class WoLayoutRazorGenerator
    {
        ...
        public WoLayoutRazorGenerator(string classModelName)
        {
        ...
        {
            BuildHeaderClass(baseContainer);

            _finalClass.AppendLine(BuildBodyClass(baseContainer));
        ...
        /// </summary>
        private void BuildHeaderClass(WoContainer container)
        {
            StringBuilder urlPage = new StringBuilder();
            urlPage.AppendLine(
                $@"@page ""/{container.Proceso}/{container.ModelType}/{container.ModelId}"""
            );
...
@*Atributo de autorizacion (solo se puede accesar si esta autenticado)*@
@attribute [Authorize]  

@*Herencia de la clase base de componentes (Permite gestionar la autenticacion y el cliente)*@
...
<h5>@Localizer[""{container.Etiqueta}""]</h5>
"
            );
        }
        ...
        [SupportedOSPlatform("windows")]
        private string BuildBodyClass(WoContainer container)
        {
            try
            {
                string body = BuildContainer(container);
                return body;
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        [SupportedOSPlatform("windows")]
        private string BuildContainer(WoContainer container)
        {
            try
            {
                ...
                strContainerCode.AppendLine(AnalizeContainer(container));
        ...
        /// <exception cref="Exception"></exception>
        [SupportedOSPlatform("windows")]
        private string AnalizeContainer(WoContainer container)
        {
            try
            {
                StringBuilder strContainerCode = new StringBuilder();
                ...
                if (container.TypeContainer == eTypeContainer.FormTabGroup)
                {
                    ...
                    foreach (WoContainer tab in container.ContainersCol)
                    {
                        ...
                        strContainerCode.AppendLine(BuildContainer(tab));
                    }
                }
                else
                {
                    ...
                    for (int row = 0; row <= container.Row; row++)
                    {
                        ...
                        for (int column = 0; column <= 12; column++)
                        {
                            ...
                            if (findItem != null)
                            {
                                strContainerCode.AppendLine(GetItemRazorCode(findItem));
                            }
                            else if (findContainer != null)
                            {
                                strContainerCode.AppendLine(BuildContainer(findContainer));
                            }
                        }
                    }
                }
        ...
        /// <exception cref="Exception"></exception>
        [SupportedOSPlatform("windows")]
        private string GetItemRazorCode(WoItem item)
        {
            try
            {
                ...
                if (item.Control == "Label")
                {
                    ...
                    {
                        razorCode = _woListGridRazor.GetCode(_identItems, item);
                    }
                }
                else if (
                ...
                {
                    razorCode = _woDetailGridRazor.GetCode(item, _identItems);
                }
